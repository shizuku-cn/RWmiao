...
public final class GameTickDispatcher {
    ...
    public static final class Registration {
        private final GameTickDispatcher owner;
        private final Method method;
        private final Callback callback;
        private volatile boolean closed;

        private Registration(GameTickDispatcher owner, Method method, Callback callback) {
            this.owner = owner;
            this.method = method;
    ...
    public GameTickDispatcher(RWmiaoModule host) {
        this.host = host;
    }

    public Registration register(Method method, Callback callback) {
        if (method == null || callback == null) {
            throw new IllegalArgumentException("tick method and callback are required");
        }
            ...
            if (entry.hook == null) {
                entry.hook = host.hookExecutable(method, chain -> {
                    ...
                    for (Callback registeredCallback : callbacks) {
                        registeredCallback.onTick(chain.getThisObject());
                    }
    ...
    public void closeAll() {
        for (Entry entry : new ArrayList<>(entries.values())) {
            synchronized (entry) {
                XposedInterface.HookHandle hook = entry.hook;
                entry.hook = null;
                entry.callbacks = new Callback[0];
                entries.remove(entry.method, entry);
                if (hook != null) {
                    try {
                        hook.unhook();
                    } catch (Throwable ignored) {
